...
class ElevatorsProblem(search.Problem):
    ...
    def __init__(self, initial):
        ...
        for eid in self.elevator_ids:
            ...
            self.elevator_reachable_sets.append(frozenset(reachable_floors))
        ...
        elevator_neighbors = []
        for left_idx, left_floors in enumerate(self.elevator_reachable_sets):
            neighbors = []
            shared_floors_row = []
            for right_idx, right_floors in enumerate(self.elevator_reachable_sets):
                shared_floors = frozenset(left_floors.intersection(right_floors))
                shared_floors_row.append(shared_floors)
                if left_idx != right_idx and shared_floors:
    ...
    def _person_lower_bound(self, person_idx, location, elevator_floors):
        ...
        if location_type == self.ON_FLOOR:
            ...
            for elevator_idx in self.person_useful_elevators[person_idx]:
                if value not in self.elevator_reachable_sets[elevator_idx]:
                    continue
    ...
    def _min_rides_from_floor(self, person_idx, floor):
        min_rides = None
        for elevator_idx in self.person_useful_elevators[person_idx]:
            if floor not in self.elevator_reachable_sets[elevator_idx]:
                continue
            rides = self.person_goal_distance[person_idx][elevator_idx]
            if rides is None:
                continue
